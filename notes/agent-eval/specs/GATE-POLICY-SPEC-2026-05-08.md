...
门禁不是判官，是反馈回路；回路的"框"本身要定期重校。以下纪律与升级/回退条件并列生效：

1. **模型/干预变更即重校**：更换 LLM provider 或模型版本、大改 system prompt、调整 judge 评分标准、启用/停用 memory 机制时，必须重跑 baseline compare 并在 session 记录中留痕——干预的有效性随模型版本衰减，昨天校准好的阈值换模型后默认失效。
2. **校准集/标注集有保质期**（criteria drift）：标注标准会在标注过程中演化。每个标注批次结束时复检指引（`SEMANTIC-FAILURE-ANNOTATION-GUIDE`）；用于修订指引的批次不再充当机制变更的验证批次（校准集与验证集分离）。
3. **小样本不报点估计结论**：比例类指标必须带分母（`x/n`）阅读；报告已提供单侧 99% Wilson 置信界（报告层展示，gate 判定暂不消费）。n < 20 时 gate 结果只作参考，不作为机制优劣结论。
4. **假失败对称面**：分析 gate FAIL 时先问"这次失败真的是 agent 的失败吗"——基础设施/超时/鉴权类失败按 `EVAL-FAILURE-TAXONOMY` 的 L1 分类剔除，再谈语义归因。
