...
fn temp_dir() -> std::path::PathBuf {
    let root = std::env::temp_dir().join(format!("amclaw_chat_adapter_test_{}", Uuid::new_v4()));
...
fn build_test_bot_with_pipeline(
    ...
    let mut bot = WeChatBot {
        ...
        task_executor: crate::task_executor::TaskExecutor::start(pipeline, db_path.to_path_buf()),
...
fn message_row(db_path: &std::path::Path, message_id: &str) -> Option<(String, String, String)> {
