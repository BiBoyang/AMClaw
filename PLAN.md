...
更新于 2026-04-13。v0.3.2 收口，v0.3.3 路线已明确。
...
- Context / Memory 层：已完成 `Trace + 最小 Context + 最小 Memory + 最小 Controller`，下一阶段主线应转向显式 `session state` 与结构化 `context pack`
...
- [ ] `context / memory` 仍处于最小版本
  - 当前已有 `session_text + current_task + recent_tasks + user_memories`
  - 但还没有显式 `session state`
  - 也还没有独立 `context pack schema`
  - 长期 memory 仍主要停留在 `user_memories`

### 下一步最值得做

1. 先收口 `Memory v3` 的语义与观测，避免设计稿与实现继续漂移
2. 引入显式 `session state`，把“当前任务进行到哪”从隐式状态变成结构化状态
3. 抽 `context pack`，把 prompt 组装从字符串拼接升级为结构化上下文包
4. 再继续把 `state/controller` 从 budget 扩到更完整的策略控制
5. 之后再评估是否值得引入更细的 budget，例如 step budget / failure budget / tool budget
...
1. 先收口现有 `Memory v3` 的语义与统计口径
2. 补显式 `session state`
3. 抽结构化 `context pack`
4. 用轻量评测驱动 runtime 稳定性增强
