...
- **C3/C4 (v0.3.3)**：上下文组装升级为结构化 `ContextPack` 单入口渲染（`src/context_pack.rs`），所有 prompt 通过 `build_context_pack` -> `render_prompt_from_context_pack` 组装，trace 新增 `context_pack_present` / `context_pack_section_count` / `context_pack_total_chars` / `context_pack_drop_reasons` 字段，并输出 `context_pack_built` / `context_pack_trimmed` 日志事件
